...
async function getStudentData(email: string) {
  ...
  /*
   * IMPORTANT:
   * zoomAccessToken / zoomRefreshToken are select:false,
   * so they never reach the frontend.
   */
  const zoomConnection = student.academyId
  ...
  return { student, academy, zoomConnection };
}
...
export default async function StudentSettingsPage({
  ...
  const { student, academy, zoomConnection } = data;
  ...
  const zoomData = zoomConnection
    ? {
        _id: String(zoomConnection._id),
        zoomConnected: Boolean((zoomConnection as any).zoomConnected),
        zoomUserId: String((zoomConnection as any).zoomUserId || ''),
        zoomAccountId: String((zoomConnection as any).zoomAccountId || ''),
        zoomEmail: String((zoomConnection as any).zoomEmail || ''),
        connectedAt: (zoomConnection as any).createdAt
          ? new Date((zoomConnection as any).createdAt).toISOString()
          : null,
      }
    : null;
