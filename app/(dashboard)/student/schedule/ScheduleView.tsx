...
export default function ScheduleView({
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      livekit: classes.filter((c) => c.hasLiveKit).length,
    };
  }, [classes, todayName]);
  ...
  const filtered = useMemo(() => {
    ...
    if (livekitOnly) {
      result = result.filter((c) => c.hasLiveKit);
    }
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky-500 via-cyan-500 to-teal-500 p-6 sm:p-8 text-white shadow-2xl shadow-cyan-500/20">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-base sm:text-lg text-cyan-100">
              {student.name}
            ...
            {todayCount > 0 && (
              <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur border border-white/25 text-sm font-semibold">
                ...
                You have <span className="text-white font-bold">{todayCount}</span> class
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition whitespace-nowrap ${
              livekitOnly
                ? 'bg-gradient-to-r from-cyan-500 to-teal-600 text-white shadow-md shadow-cyan-500/25'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
          >
      ...
      {classes.length === 0 ? (
        <div className="bg-white rounded-3xl p-10 sm:p-14 text-center border-2 border-dashed border-sky-200 shadow-sm">
          ...
          <p className="text-slate-500 mt-2 max-w-md mx-auto text-sm">
            Your academy has not assigned any classes to you yet. Once your
            teacher schedules a class, it will appear here.
          </p>
        </div>
      ) : filtered.length === 0 ? (
      ...
      <div className="relative p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          {hasLiveKit ? (
            ...
            <span className="shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 ring-1 ring-amber-200 text-[10px] font-bold uppercase tracking-wider">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-700 mb-3">
          ...
          <span className="font-semibold truncate">{row.teacherName}</span>
        </div>
            ...
            <Link
              ...
              className={`group/btn inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white transition shadow-md ${
                isToday
                  ...
                  : 'bg-gradient-to-r from-sky-600 to-cyan-600 hover:shadow-lg hover:shadow-cyan-500/30 shadow-cyan-500/25'
              }`}
            >
