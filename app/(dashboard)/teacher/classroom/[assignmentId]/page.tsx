...
export default async function TeacherClassroomPage({
  ...
  const teacher = await Teacher.findOne({ email }).select('_id academyId name email').lean();
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white border border-slate-200 shadow-sm">
          ...
          <div className="relative p-8 sm:p-12 text-center">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Zoom Classroom Not Ready</h1>
            <p className="mt-3 text-sm sm:text-base text-slate-500 max-w-lg mx-auto leading-relaxed">
              This class does not have a Zoom meeting configured yet. Please ask the academy owner to configure the permanent Zoom meeting.
            </p>
  ...
  const classroomData = {
    ...
    courseName: String((course as any)?.name || (course as any)?.title || 'Online Class'),
    daysOfWeek: Array.isArray(assignment.daysOfWeek) ? assignment.daysOfWeek.map((d: any) => String(d)) : [],
    ...
    // ✅ Book info (agar Course model mein hai)
    courseId: String((course as any)?._id || assignment.courseId || ''),
    totalPages: Number((course as any)?.totalPages || 0),
  };

  // ✅ Days check (client-side pe bhi kar sakte hain, magar filhaal server pe hi rehne dein)
  const todayName = new Date().toLocaleDateString('en-US', { weekday: 'long' });
  const isToday = classroomData.daysOfWeek.length === 0 || classroomData.daysOfWeek.includes(todayName);
    ...
    <div className="mx-auto max-w-[1600px] space-y-4 sm:space-y-5">
      <div className="flex items-center gap-2 text-xs sm:text-sm">
        <Link href="/teacher/classes"
          ...
          <ArrowLeft className="h-3.5 w-3.5 group-hover:-translate-x-0.5 transition-transform" />My Classes
        </Link>
        ...
        <div className="relative z-10 p-5 sm:p-6 lg:p-8">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-white/95 text-[11px] font-semibold">
              <Video className="h-3.5 w-3.5 text-sky-400" />Zoom Classroom
            </span>
            {isToday && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-rose-500 to-pink-600 text-white text-[11px] font-bold">
            ...
            {classroomData.zoomProvider && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-400/30 text-emerald-200 text-[11px] font-semibold capitalize">
                <Sparkles className="h-3.5 w-3.5" />{classroomData.zoomProvider}
              </span>
            )}
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-white/70">
            <span className="inline-flex items-center gap-1.5">
              ...
              <span className="font-semibold text-white">{classroomData.studentName}</span>
              {classroomData.fatherName && <span className="text-white/50">· {classroomData.fatherName}</span>}
            </span>
            <span className="inline-flex items-center gap-1.5">
              <Clock className="h-4 w-4 text-white/40" />
              <span className="font-mono font-semibold text-white">{classroomData.startTime || '--:--'}</span>
              <span className="text-white/40">–</span>
              <span className="font-mono font-semibold text-white">{classroomData.endTime || '--:--'}</span>
            </span>
          </div>
          ...
          <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            ...
            <HeroStat icon={<Calendar className="h-4 w-4" />} label="Days"
              value={classroomData.daysOfWeek.length > 0 ? classroomData.daysOfWeek.join(', ') : 'Every day'} tone="emerald" />
            <HeroStat icon={<BookOpen className="h-4 w-4" />} label="Status" value={classroomData.status} tone="amber" capitalize />
          </div>
        </div>
      </div>

      {/* ✅ Saare props pass kiye */}
  ...
  return (
    <div className="rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 p-3">
      <div className="flex items-center gap-1.5 mb-1">
        <span className={`h-5 w-5 rounded-md flex items-center justify-center border ${toneClasses}`}>{icon}</span>
        <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">{label}</p>
      </div>
      <p className={`text-xs sm:text-sm font-bold text-white truncate ${capitalize ? 'capitalize' : ''}`} title={value}>
        {value}
      </p>
    </div>
  );
