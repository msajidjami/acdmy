...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !email || !contactNumber) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields', message: 'نام، ای میل اور رابطہ نمبر درکار ہیں' },
        { status: 400 }
      );
    }

    // اونر بنائیں – referralCode خود بخود pre-save میں بن جائے گا
    ...
    // اگر کسی وجہ سے referralCode نہ بنا ہو تو فال بیک
    if (!owner.referralCode) {
      return NextResponse.json(
        { success: false, error: 'Referral code generation failed' },
        { status: 500 }
      );
    }
    ...
    // ڈپلیکیٹ ای میل یا ریفرل کوڈ
    if (error.code === 11000) {
      const field = Object.keys(error.keyValue || {})[0];
      const value = error.keyValue?.[field];
      const msg = field === 'email' ? 'یہ ای میل پہلے سے موجود ہے' : 'ریفرل کوڈ ڈپلیکیٹ ہو گیا';
      return NextResponse.json(
        { success: false, error: 'Duplicate entry', message: msg },
        { status: 400 }
      );
    }

    // ویلیڈیشن ایرر (جیسے required فیلڈ)
    if (error.name === 'ValidationError') {
      return NextResponse.json(
        { success: false, error: 'Validation failed', message: 'درکار معلومات مکمل نہیں' },
        { status: 400 }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const ownersWithLinks = owners.map((owner: any) => ({
      ...owner,
    ...
    return NextResponse.json(
      { success: false, error: 'Server error', message: 'اونرز لوڈ نہیں ہو سکے' },
      { status: 500 }
    );
  }
}