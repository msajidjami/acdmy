...
const SITE_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://www.ilmora786.com';
...
export const metadata: Metadata = {
  ...
  description: 'Discover verified online academies worldwide. Create your own academy, manage teachers and students, and conduct live classes with interactive whiteboards.',
  keywords: ['online academy', 'islamic academy', 'live classes', 'online tutoring', 'Quran classes', 'whiteboard classroom', 'online education platform', 'academy management software', 'teacher dashboard', 'student learning platform'],
  ...
  openGraph: {
    ...
    description: 'Discover verified online academies. Create your own academy, manage teachers and students, and conduct live classes.',
    images: [{ url: `${SITE_URL}/og-image.png`, width: 1200, height: 630, alt: SITE_NAME }],
  },
  twitter: {
    card: 'summary_large_image',
    title: `${SITE_NAME} — Discover Online Academies`,
    description: 'Create your academy, manage teachers & students, run live classes.',
...
type UserAcademy = { id: string; name: string; slug: string; };
...
async function getCurrentUser(): Promise<CurrentUser | null> {
  try {
    ...
    if (!user) return null;
    return { id: String((user as any)._id), name: String((user as any).name || ''), email: String((user as any).email || ''), role: String((user as any).role || '') };
  } catch { return null; }
}
...
async function getUserAcademy(userId: string): Promise<UserAcademy | null> {
  try {
    await connectDB();
    const academy = await Academy.findOne({ ownerId: userId }).select('_id name slug').lean();
    if (!academy) return null;
    return { id: String((academy as any)._id), name: String((academy as any).name || ''), slug: String((academy as any).slug || '') };
  } catch { return null; }
}
...
export default async function HomePage() {
  ...
  const showCreateAcademy = Boolean(user && user.role === 'owner' && !userAcademy);
